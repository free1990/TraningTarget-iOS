...
#pragma mark - force to set text draw color to clear color
    ...
    float fixedHeight = self.fixedHeight > 0 ? self.fixedHeight:rect.size.height;
    CGRect fixedHeightRect = CGRectMake(rect.origin.x, rect.origin.y, rect.size.width, fixedHeight);
    ...
    if(_badgeValue != 0 || _displayIfZero) {
        ...
        [_outlineColor set];
        CGContextFillEllipseInRect(context, fixedHeightRect);
        
        [_badgeColor set];
        CGContextFillEllipseInRect(context, CGRectInset(fixedHeightRect, _outlineWidth + 1.0, _outlineWidth + 1.0));
        ...
        CGRect lblRect = CGRectMake(fixedHeightRect.origin.x, (fixedHeightRect.size.height / 2.0) - (numberSize.height / 2.0), fixedHeightRect.size.width, numberSize.height);
